...
ORDER = [
    x.strip()
    for x in os.getenv("LLM_ORDER", ",".join(DEFAULT_ORDER)).split(",")
    if x.strip()
]
ROUTER_ORDER = [
    x.strip()
    for x in os.getenv("ROUTER_ORDER", ",".join(DEFAULT_ROUTER_ORDER)).split(",")
    if x.strip()
]
...
async def _classify_tier(
    req: ChatRequest, role: str, router_pool: RouterPool, prompt_text: str
):
    ...
    for name in router_pool.candidates():
        ...
        try:
            ...
            tokens = (result.get("input_tokens") or 0) + (
...
async def chat(req: ChatRequest):
    ...
    if req.auto_route and not req.provider:
        router_decision = await _classify_tier(
            req, req.auto_route, router_pool, prompt_text
        )
        ...
        candidates = (
            router.candidates(req.provider) if req.provider else list(router.order)
        )

    if req.provider and not candidates:
        raise HTTPException(
            400,
            f"unknown provider '{req.provider}'. Try one of: {list(router.providers)} or shortcuts {list(SHORTCUTS)}",
        )
    ...
    if explicit_override and len(candidates) == 1:
        ...
        while time.time() < deadline:
            ...
            if name is not None:
                break
            cd = router.state[candidates[0]].snapshot(LIMITS[candidates[0]])[
                "cooldown_remaining"
            ]
            if cd <= 0 or cd > 30:
                break
    ...
    for _ in range(len(candidates) + 1):
        ...
        try:
            ...
            if (
                req.response_format
                and req.response_format.schema_
                and not result["tool_calls"]
            ):
                try:
                    parsed = _validate_structured(
                        result["text"], req.response_format.schema_
                    )
                    ...
                    try:
                        ...
                        raise HTTPException(
                            503, f"structured output failed validation: {ve2}"
                        )
            ...
            all_attempts.append(
                {"provider": name, "reason": f"exception: {str(e)[:120]}"}
            )
...
@app.get("/v1/calls")
async def calls(
    limit: int = 100, provider: Optional[str] = None, status: Optional[str] = None
):
