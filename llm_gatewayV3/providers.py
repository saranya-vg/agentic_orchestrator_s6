...
class OpenAICompatProvider(BaseProvider):
    ...
    async def chat(
        ...
        *,
        ...
        # V3 compatibility: newer OpenAI models (o1, o3, etc) require
        # max_completion_tokens instead of max_tokens.
        is_new_openai = self.name == "openai" and any(
            h in m.lower() for h in ("o1", "o3", "o4", "gpt-5")
        )
        if is_new_openai:
        ...
        if tools:
            ...
            if tool_choice is not None:
                body["tool_choice"] = (
                    tool_choice if isinstance(tool_choice, (str, dict)) else "auto"
                )
        ...
        async with httpx.AsyncClient(timeout=180) as c:
            ...
            if r.status_code != 200:
                ...
                if r.status_code != 200 and "json_schema" in (
                    ...
                    if "json" not in str(body.get("messages", "")).lower():
                        if body["messages"] and body["messages"][0]["role"] == "system":
                            ...
                            body["messages"].insert(
                                0, {"role": "system", "content": "Respond in JSON."}
                            )
                    r = await c.post(
                        f"{self.base_url}/chat/completions",
                        headers=self._headers(),
        ...
        async with httpx.AsyncClient(timeout=180) as c:
            async with c.stream(
                ...
                if r.status_code != 200:
                    ...
                    raise ProviderError(
                        f"{self.name} HTTP {r.status_code}: {text}",
                        status=r.status_code,
                    )
                async for line in r.aiter_lines():
                    ...
                    try:
                        ...
                        if delta.get("tool_calls"):
                            yield "[[TOOL_CALL_DELTA]] " + json.dumps(
                                delta["tool_calls"]
                            )
                    except Exception:
                        continue
...
class GeminiProvider(BaseProvider):
    ...
    def __init__(self, api_key, model, cache_store):
        super().__init__(
            api_key, model, "https://generativelanguage.googleapis.com/v1beta"
        )
    ...
    async def chat(
        ...
        *,
        ...
        if cache_name:
            ...
            remaining_sys = (
                "\n".join(b["text"] for b in blocks if not b["cache"])
                if has_cache_marker
                else ""
            )
            if remaining_sys:
        ...
        if response_format:
            ...
            if rf.get("schema"):
                ...
                body["generationConfig"]["responseSchema"] = _gemini_clean_schema(
                    rf["schema"]
                )
            elif rf.get("type") == "json_object":
                body["generationConfig"]["responseMimeType"] = "application/json"
        ...
        async with httpx.AsyncClient(timeout=180) as c:
            ...
            if not cands:
                raise ProviderError(
                    f"gemini no candidates: {json.dumps(d)[:200]}",
                    status=200,
                    retryable=True,
                )
    ...
    async def chat(
        ...
        *,
        ...
        if prompted_fallback:
            system_text = (
                system_text + "\n\n" if system_text else ""
            ) + _prompted_tool_system(tools)

        body = {
            "model": m,
            "messages": self._translate_messages(
                messages, system_text, prompted_fallback=prompted_fallback
            ),
        ...
        async with httpx.AsyncClient(timeout=600) as c:
            ...
            dialect = (
                "native"
                if native
                else ("prompted_fallback" if prompted_fallback else "none")
            )
...
def build_providers(cache_store):
    ...
    if k := os.getenv("GEMINI_API_KEY"):
        out["gemini"] = GeminiProvider(
            k, os.getenv("GEMINI_MODEL", "gemini-2.5-flash"), cache_store
        )
    if k := os.getenv("NVIDIA_API_KEY"):
        out["nvidia"] = NvidiaProvider(
            k, os.getenv("NVIDIA_MODEL", "deepseek-ai/deepseek-v3.2")
        )
    ...
    if k := os.getenv("CEREBRAS_API_KEY"):
        out["cerebras"] = CerebrasProvider(
            k, os.getenv("CEREBRAS_MODEL", "zai-glm-4.7")
        )
    ...
    if k := os.getenv("GITHUB_ACCESS_TOKEN"):
        out["github"] = GitHubProvider(
            k, os.getenv("GITHUB_MODEL", "openai/gpt-4.1-mini")
        )
    if om := os.getenv("OLLAMA_MODEL"):
        out["ollama"] = OllamaProvider(
            om, os.getenv("OLLAMA_URL", "http://localhost:11434")
        )
    return out
...
def build_router_providers():
    ...
    if k := os.getenv("OPENAI_API_KEY"):
        out["openai"] = OpenAIProvider(
            k, os.getenv("ROUTER_OPENAI_MODEL", ROUTER_DEFAULTS["openai"])
        )
    if k := os.getenv("CEREBRAS_API_KEY"):
        out["cerebras"] = CerebrasProvider(
            k, os.getenv("ROUTER_CEREBRAS_MODEL", ROUTER_DEFAULTS["cerebras"])
        )
    if k := os.getenv("GROQ_API_KEY"):
        out["groq"] = GroqProvider(
            k, os.getenv("ROUTER_GROQ_MODEL", ROUTER_DEFAULTS["groq"])
        )
    if k := os.getenv("NVIDIA_API_KEY"):
        out["nvidia"] = NvidiaProvider(
            k, os.getenv("ROUTER_NVIDIA_MODEL", ROUTER_DEFAULTS["nvidia"])
        )
    if k := os.getenv("GITHUB_ACCESS_TOKEN"):
        out["github"] = GitHubProvider(
            k, os.getenv("ROUTER_GITHUB_MODEL", ROUTER_DEFAULTS["github"])
        )
    return out
