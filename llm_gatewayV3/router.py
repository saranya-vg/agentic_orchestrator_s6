...
class RateState:
    ...
    def can_use(self, limits, est_tokens=0):
        ...
        if now < self.unavailable_until:
            return (
                False,
                f"backoff: {self.unavailable_reason} ({self.unavailable_until - now:.0f}s left)",
            )
        ...
        if (
            "tokens_per_day" in limits
            and self.tokens_today + est_tokens > limits["tokens_per_day"]
        ):
    ...
    def snapshot(self, limits):
        ...
        return {
            ...
            "backoff_reason": self.unavailable_reason
            if now < self.unavailable_until
            else "",
        }
    ...
    def pick(self, est_tokens, candidates, required_caps: list[str] | None = None):
        ...
        for name in candidates:
            ...
            if required_caps:
                ...
                if missing:
                    attempts.append(
                        {"provider": name, "reason": f"skipped:no_{missing[0]}"}
                    )
                    continue
            if est_tokens > limits["max_ctx"]:
                attempts.append(
                    {
                        "provider": name,
                        "reason": f"prompt {est_tokens} > max_ctx {limits['max_ctx']}",
                    }
                )
                continue
...
DEFAULT_ROUTER_ORDER = ["openai", "cerebras", "groq", "nvidia", "github"]
