...
def _duckduckgo_html_search(query: str, max_results: int) -> list[dict]:
    ...
    results = []
    parts = text.split('<div class="result">')[1:]
    for part in parts:
        if len(results) >= max_results:
            break
        url_match = re.search(r'<a[^>]+href="([^"]+)"', part)
        title_match = re.search(r'<a[^>]+class="result__a"[^>]*>(.*?)</a>', part, re.S)
        snippet_match = re.search(r'<a[^>]+class="result__snippet"[^>]*>(.*?)</a>', part, re.S)
